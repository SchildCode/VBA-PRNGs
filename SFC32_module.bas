...
Public Function Bryc32_U32^()
    ...
    'Ref. https://github.com/bryc/code/blob/master/jshash/PRNGs.md
    Dim left^, right^
'---
    a = (a + b) And MASK32         'a += b
    b = (b Xor a) And MASK32       'b ^= a
    c = (c + 1) And MASK32         'c += 1
    d = (d + a) And MASK32         'd += a
    left = (d * BIT11) And MASK32  '}
    right = d \ BIT21              '} d = rotl(d, 11)
    d = (left Or right) And MASK32 '}
    a = (a + d) And MASK32         'a += d
